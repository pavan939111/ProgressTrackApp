  ...
  return (
    <div className="space-y-8 pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold bg-muted border border-border text-primary"
          >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Avg completion', value: `${analytics.avgDailyCompletion}%`, color: 'text-primary' },
          { label: 'Consistency', value: `${analytics.consistencyScore}%`, color: 'text-secondary' },
          { label: 'Today done', value: `${analytics.completedTasks}/${analytics.totalTasks}`, color: 'text-accent' },
          { label: 'Best day', value: analytics.bestDay.slice(5) || 'â€”', color: 'text-primary' },
        ].map((card) => (
          <div
            key={card.label}
            className="glass-panel rounded-2xl p-5 border border-border bg-background/60"
          >
      ...
      <div className="glass-panel rounded-3xl p-6 md:p-8 border border-border bg-background/80">
        <h2 className="text-sm font-bold uppercase tracking-wider text-muted-foreground flex items-center gap-2 mb-6">
          <TrendingUp className="w-4 h-4 text-primary" /> Daily completion trend
        </h2>
        <div className="flex items-end gap-1.5 h-40">
          {analytics.dailySeries.map((d) => (
            <div key={d.date} className="flex-1 flex flex-col items-center gap-1 group">
              ...
              />
              <span className="text-[9px] text-muted-foreground rotate-0 md:rotate-0">
                {d.date.slice(8)}
              </span>
            </div>
          ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="glass-panel rounded-3xl p-6 border border-border bg-background/80 space-y-4">
            ...
            return (
              <div key={s.session}>
                <div className="flex justify-between text-xs mb-1">
                  <span className="text-muted-foreground font-semibold">{s.session}</span>
                  <span className="text-muted-foreground">
                    {s.completed}/{s.total} Â· {pct}%
                  </span>
                </div>
          ...
          {analytics.weakestSession && (
            <p className="text-xs text-accent/90 pt-2">
              Weakest block: <strong>{analytics.weakestSession}</strong>
            </p>
          )}
        </div>

        <div className="glass-panel rounded-3xl p-6 border border-border bg-background/80 space-y-3">
          ...
          {analytics.insights.map((insight) => (
            <div
              key={insight}
              className="p-3.5 rounded-2xl border border-border bg-muted/60 text-sm text-slate-200"
            >
              {insight}
            </div>
          ))}
